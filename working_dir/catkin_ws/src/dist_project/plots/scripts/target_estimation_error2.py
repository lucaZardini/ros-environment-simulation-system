...
for bagfile in bagfiles:
    bagfile = os.path.join(os.path.join(bag_root, bag_folder), os.path.join(bag_experiment, bagfile))

    # Create a figure for the plots (one plot for each target)
    fig, axes = plt.subplots(nrows=len(ground_truth_positions), ncols=1, figsize=(10, 15))  # Three targets, one plot per target
